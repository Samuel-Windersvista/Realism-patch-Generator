...
### 新增
- 新增 rule_range_editor.py 图形化规则范围编辑器，可直接浏览并修改武器、武器精修、附件、装备、弹药规则文件中的二元范围值。
- 新增 规则范围编辑器.bat，便于在 Windows 下直接启动图形界面编辑器。

### 变更
- 规则范围编辑器支持按规则类别/档位浏览，并通过双击表格单元格直接编辑最小值与最大值。
- 规则范围编辑器新增字段说明面板；选中任意规则项后，可直接查看字段作用、数值方向提示和当前规则类别说明，降低调参理解成本。
- 规则范围编辑器保存时会自动备份被修改的规则文件，避免调参失误后难以回滚。
- 规则范围编辑器新增“生成补丁”按钮，可直接在当前 Python 环境下一键运行生成器并输出到 output 目录。

### 文档
- README.md 新增 GUI 规则编辑器的启动方式、可编辑规则范围、字段说明面板，以及自动备份和一键生成补丁的使用说明。
