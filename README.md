...
**EFT 现实主义数值生成器 v3.11** 是一个功能强大的Python脚本，用于为《逃离塔科夫》(Escape from Tarkov)的 SPT3.11.4 现实主义 MOD (Realism Mod) 自动重建并生成数值配置。它可以根据 `input/` 文件夹中的物品数据，使用预定义模板快速生成规范化配置文件。
...
| 版本 | 日期 | 主要更新 |
|------|------|---------|
| **v3.11** | 2026-03-11 | ✅ **瞄具误判收敛修复** - 继续修正 `ScopeTemplates` 中红点/全息镜误判为高倍率的问题，补充 `1p87` / `comp_m4` / `boss_xe` 等关键词，并优化 `ModType=sight` 在 `ScopeTemplates` 下的兜底顺序。 |
| **v3.10** | 2026-03-10 | 🎯 **瞄具分档识别增强** - 优化 `ScopeTemplates` 的识别链路，`ModType=sight` 改为名称关键词优先再回退 parent 映射，增强红点/全息与高倍镜关键词与倍率识别，减少全量误判为同一档位。 |
| **v3.9** | 2026-03-10 | 🛠️ **武器/附件规则对齐修正** - 调整 `stockless` 武器二级修正规则，避免冲锋枪后坐被异常抬高；同步修正 `mount` 档位热量字段策略，避免无意义的 `HeatFactor/CoolFactor = 0.01` 自动注入。 |
| **v3.8** | 2026-03-10 | 🧪 **弹药第三层模板对齐** - 依据 `现实主义物品模板/ammo/ammoTemplates.json`（402条）重建第三层弹种分类，新增 `tracer` / `shot_shell_payload` / `ball_standard` 档，并将匹配升级为词元精确命中，修复短关键词子串误判。 |
| **v3.7** | 2026-03-10 | 🧠 **弹药三层精细化规则** - 穿深分层升级为 `11` 级（`1~130`），新增第三层“弹种型号”修正（含 `M995` / `M855A1` 指定档）与无点口径关键词兼容，修复同口径高穿弹关系反转问题。 |
