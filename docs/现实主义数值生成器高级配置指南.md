...
如果你的目标只是调整区间数值，而不是改识别逻辑或导出逻辑，优先使用 rule_range_editor.py，而不是直接编辑源码。
...
当前 Gear 已有独立规则文件 gear_rule_ranges.py，并按 gear_profile 应用范围，而不是只做全局夹紧。

当前 gear 调参入口主要是：

- gear_rule_ranges.py：调整装备档位范围
- generate_realism_patch.py：仅在 gear 识别逻辑、规则顺序或夹紧策略需要变化时再改

如果只是改 Comfort、SpallReduction、ReloadSpeedMulti、speedPenaltyPercent 这类范围，优先使用 GUI 编辑器。
...
4. GUI 编辑器相关说明是否已同步到 README 和使用文档
5. CHANGELOG.md、批处理横幅、主脚本横幅是否同步了版本号
